...
def face_swap(orig_image, down_scale):
    ...
    for (face, resized_image) in facelist:
        ...
        warped_resized_image = cv2.warpAffine(
            resized_image, mat, (64, 64)) / 255.0
        ...
        new_face = numpy.clip(numpy.squeeze(
            figure[0]) * 255.0, 0, 255).astype('uint8')
...
def process_video(in_filename, out_filename, keep_audio=True, down_scale=2):
    ...
    if keep_audio == True:
        ...
        clip.audio.write_audiofile(
            "./temp/src_audio.mp3", progress_bar=False, verbose=False)
    ...
    vidwriter = cv2.VideoWriter("./temp/proc_video.avi", cv2.VideoWriter_fourcc(
        'M', 'J', 'P', 'G'), fps, (width // down_scale, height // down_scale))
    ...
    for i in tqdm(range(frames_count)):
        ...
        try:
            ...
            new_image = cv2.resize(
                image, (width // down_scale, height // down_scale))
    ...
    if keep_audio == True:
        ...
        video.write_videofile(
            out_filename, audio="./temp/src_audio.mp3", progress_bar=False, verbose=False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Apply face swap on YouTube video.')
    parser.add_argument(
        "--url", help="YouTube Video URL (e.g. https://www.youtube.com/watch?v=XnbCSboujF4)")
    parser.add_argument("--start", default=0, type=int,
                        help="Start time in seconds (-1 for full video)")
    parser.add_argument("--stop", default=-1, type=int,
                        help="Stop time in seconds (-1 for full video)")
    parser.add_argument("--gif", default=False, type=bool,
                        help="Export as gif instead of video with audio")
